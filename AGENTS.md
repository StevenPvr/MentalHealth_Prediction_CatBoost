...
## 0. Project Setup (Before Starting)

- **Logging Setup**: At the beginning of each project, create a centralized log file and create `pipeline.log` before recensing anything
- Centralized logging should be configured early to track all project activities
...
- Use pytest fixtures
- **Required**: All test files must be executable individually by running the `.py` file directly (e.g., `python test_module.py`)
- **Required**: Each project must have:
  - Integration tests (test interactions between modules)
  - End-to-end (e2e) tests (test complete workflows/pipelines)
